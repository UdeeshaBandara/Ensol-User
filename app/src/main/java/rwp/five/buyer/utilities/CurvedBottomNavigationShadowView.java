...
public class CurvedBottomNavigationShadowView extends View {
    private Path mPath;
    private Paint mPaint;

    public final int CURVE_CIRCLE_RADIUS = 65;
    public final int CURVE_PARAM1 = 70;
    public final int CURVE_START_PARAM = 15;
    ...
    public CurvedBottomNavigationShadowView(Context context) {
        super(context);
        init();
    }

    public CurvedBottomNavigationShadowView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public CurvedBottomNavigationShadowView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mFirstCurveStartPoint.set((mNavigationBarWidth / 2) - (CURVE_CIRCLE_RADIUS * 2) - CURVE_START_PARAM, 0);
        // the coordinates (x,y) of the end point after curve
        mFirstCurveEndPoint.set(mNavigationBarWidth / 2, CURVE_CIRCLE_RADIUS + (CURVE_CIRCLE_RADIUS / 2));
        // same thing for the second curve
        mSecondCurveStartPoint = mFirstCurveEndPoint;
        mSecondCurveEndPoint.set((mNavigationBarWidth / 2) + (CURVE_CIRCLE_RADIUS * 2) + CURVE_START_PARAM, 0);
